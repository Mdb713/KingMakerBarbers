    ...
    <section class="bg-dark text-gray-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4">
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-heading font-bold text-gold">Carrito de Compras</h1>
                <a href="{{ route('productos') }}"
                    class="text-gold font-body hover:text-yellow-500 transition-colors flex items-center gap-2 text-sm sm:text-base">
            ...
            @if (session('success'))
                <div
                    class="mb-4 sm:mb-6 bg-green-500/20 border border-green-500 text-green-400 p-3 sm:p-4 rounded-lg shadow-lg transition-all text-sm sm:text-base">
                    {{ session('success') }}
                </div>
            @endif
            @if (session('error'))
                <div class="mb-4 sm:mb-6 bg-red-500/20 border border-red-500 text-red-400 p-3 sm:p-4 rounded-lg shadow-lg transition-all text-sm sm:text-base">
                    {{ session('error') }}
                </div>
            @endif
            ...
            @if (count($carrito) > 0)
                <div class="grid lg:grid-cols-3 gap-6 lg:gap-8">
                    <div class="lg:col-span-2 space-y-4">
                        <div class="bg-medium-gray p-4 sm:p-6 rounded-xl border border-gold/10">
                            <div class="flex justify-between items-center mb-4">
                                <h2 class="text-lg sm:text-xl font-semibold font-body text-gold">Productos ({{ count($carrito) }})</h2>
                                <button id="btnVaciarCarrito"
                                    class="text-red-400 hover:text-red-300 font-body text-xs sm:text-sm transition-colors flex items-center gap-1">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                    <span class="hidden sm:inline">Vaciar carrito</span>
                                </button>
                            </div>
                            ...
                            <div class="space-y-3 sm:space-y-4">
                                @foreach ($carrito as $id => $producto)
                                    <div
                                        class="bg-dark-gray p-3 sm:p-4 rounded-lg flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 hover:bg-dark-gray/70 transition-all border border-gold/5">
                                        <img src="{{ asset($producto['imagen_url']) }}" alt="{{ $producto['nombre'] }}"
                                            class="w-full sm:w-20 h-32 sm:h-20 rounded-lg object-cover">

                                        <div class="flex-1 w-full">
                                            <h3 class="text-base sm:text-lg font-semibold text-gray-100">
                                                {{ $producto['nombre'] }}</h3>
                                            <p class="text-xs sm:text-sm font-body text-gray-400 mt-1">Precio unitario: <span
                                                    class="text-gold">{{ $producto['precio'] }}€</span></p>
                                            <div class="flex items-center gap-3 mt-2">
                                                <span class="text-xs sm:text-sm font-body text-gray-400">Cantidad:</span>
                                                <div class="flex items-center gap-2 bg-medium-gray rounded-lg px-2 sm:px-3 py-1">
                                                    <button
                                                        class="text-gray-400 hover:text-gold transition-colors text-lg">-</button>
                                                    <span
                                                        class="text-gold font-semibold w-6 sm:w-8 text-center">{{ $producto['cantidad'] }}</span>
                                                    <button
                                                        class="text-gray-400 hover:text-gold transition-colors text-lg">+</button>
                                                </div>
                                            </div>
                                        ...
                                        <div class="flex sm:flex-col justify-between sm:text-right w-full sm:w-auto items-center sm:items-end">
                                            <p class="text-lg sm:text-xl font-bold text-gold">
                                                {{ $producto['precio'] * $producto['cantidad'] }}€</p>
                                            <form action="{{ route('carrito.eliminar') }}" method="POST" class="mt-0 sm:mt-2">
                                                @csrf
                                                <input type="hidden" name="producto_id" value="{{ $id }}">
                                                <button type="submit"
                                                    class="text-red-400 font-body hover:text-red-300 text-xs sm:text-sm transition-colors flex items-center gap-1">
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-medium-gray p-4 sm:p-6 rounded-xl border border-gold/10 lg:sticky lg:top-32">
                            <h2 class="text-xl sm:text-2xl font-heading font-bold text-gold mb-4 sm:mb-6">Resumen del Pedido</h2>
                            <div class="space-y-2 sm:space-y-3 mb-4 sm:mb-6 pb-4 sm:pb-6 border-b border-gold/10">
                            ...
                            <div class="mb-4 sm:mb-6 pb-4 sm:pb-6 border-b border-gold/10">
                                <div class="flex justify-between items-center">
                                    <span class="text-lg sm:text-xl font-semibold">Total:</span>
                                    <span
                                        class="text-2xl sm:text-3xl font-bold text-gold">{{ number_format(collect($carrito)->sum(fn($p) => $p['precio'] * $p['cantidad']) * 1.21, 2) }}€</span>
                                </div>
                            </div>

                            @auth
                                <div class="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                                    <label class="block text-gray-300 font-semibold mb-2 sm:mb-3 text-sm sm:text-base">Método de pago:</label>

                                    <label
                                        class="flex items-center p-3 sm:p-4 bg-dark-gray rounded-lg cursor-pointer hover:bg-dark-gray/70 transition-all border-2 border-transparent has-[:checked]:border-gold">
                                        <input type="radio" name="metodo_pago" value="tarjeta"
                                            class="w-4 h-4 sm:w-5 sm:h-5 text-gold accent-gold flex-shrink-0">
                                        <div class="ml-3 flex-1">
                                            <div class="flex items-center gap-2">
                                                <svg class="w-5 h-5 sm:w-6 sm:h-6 text-gold flex-shrink-0" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                                                </svg>
                                                <span class="font-semibold text-sm sm:text-base">Tarjeta de crédito/débito</span>
                                            </div>
                                        </div>
                                    </label>
                                    ...
                                    <label
                                        ...
                                        <div class="ml-3 flex-1">
                                            <div class="flex items-center gap-2">
                                                <svg class="w-5 h-5 sm:w-6 sm:h-6 text-gold flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                                                    <path
                                                        d="M7.076 21.337H2.47a.641.641 0 0 1-.633-.74L4.944.901C5.026.382 5.474 0 5.998 0h7.46c2.57 0 4.578.543 5.69 1.81 1.01 1.15 1.304 2.42 1.012 4.287-.023.143-.047.288-.077.437-.983 5.05-4.349 6.797-8.647 6.797h-2.19c-.524 0-.968.382-1.05.9l-1.12 7.106zm14.146-14.42a3.35 3.35 0 0 0-.607-.541c-.013.076-.026.175-.041.254-.93 4.778-4.005 7.201-9.138 7.201h-2.19a.563.563 0 0 0-.556.479l-1.187 7.527h-.506l-.24 1.516a.56.56 0 0 0 .554.647h3.882c.46 0 .85-.334.922-.788.06-.26.76-4.852.816-5.09a.932.932 0 0 1 .923-.788h.58c3.76 0 6.705-1.528 7.565-5.946.36-1.847.174-3.388-.777-4.471z" />
                                                </svg>
                                                <span class="font-semibold text-sm sm:text-base">PayPal</span>
                                            </div>
                                        </div>
                                    </label>
                                    ...
                                    <label
                                        ...
                                        <div class="ml-3 flex-1">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="font-semibold text-sm sm:text-base">Efectivo contra entrega</span>
                                            </div>
                                        </div>
                                    </label>
                                </div>
                                ...
                                <button id="btnPagar"
                                    class="w-full bg-gold hover:bg-yellow-500 text-dark font-bold py-3 sm:py-4 rounded-lg transition-all shadow-lg transform hover:scale-105 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 text-sm sm:text-base">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <p class="text-xs text-gray-400 text-center mt-3 sm:mt-4">
                            ...
                            @else
                                <div class="text-center py-6 sm:py-8">
                                    <svg class="w-12 h-12 sm:w-16 sm:h-16 text-gold mx-auto mb-3 sm:mb-4" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                    </svg>
                                    <p class="text-gray-400 mb-3 sm:mb-4 text-sm sm:text-base">Debes iniciar sesión para continuar con tu compra</p>
                                    <a href="{{ route('login') }}"
                                        class="inline-block bg-gold hover:bg-yellow-500 text-dark font-bold py-2 sm:py-3 px-5 sm:px-6 rounded-lg transition-all text-sm sm:text-base">
                                        Iniciar Sesión
                                    </a>
                                </div>
                            @endauth
                        ...
                        <div class="mt-4 sm:mt-6 bg-medium-gray p-3 sm:p-4 rounded-xl border border-gold/10">
                            <div class="flex items-start gap-3 mb-3">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gold mt-0.5 flex-shrink-0" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                                </svg>
                                <div>
                                    <p class="font-semibold text-xs sm:text-sm">Envío gratis</p>
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="font-semibold text-xs sm:text-sm">Factura digital</p>
            ...
            @else
                <div class="text-center py-12 sm:py-20">
                    <svg class="w-24 h-24 sm:w-32 sm:h-32 text-gray-600 mx-auto mb-4 sm:mb-6" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                    </svg>
                    <h2 class="text-xl sm:text-2xl font-title text-gray-400 mb-3 sm:mb-4">Tu carrito está vacío</h2>
                    <p class="text-sm sm:text-base text-gray-500 mb-6 sm:mb-8">Añade productos para comenzar tu compra</p>
                    <a href="{{ route('productos') }}"
                        class="inline-block bg-gold hover:bg-yellow-500 text-dark font-bold py-2 sm:py-3 px-6 sm:px-8 rounded-lg transition-all text-sm sm:text-base">
                        Ver Productos
                    </a>
                </div>
            @endif
